...
wiki/                   ← repo root (this file lives here)
  ...
  <subwiki>/            ← one directory per research domain (see Sub-wikis below)
...
<!-- List each sub-wiki, its scope, and related sub-wikis. Example: -->

| Directory | Scope | Related |
|---|---|---|
| `tsa/` | Time series analysis, forecasting, state space models, ARIMA, spectral methods | `bayes/`, `methods/` |
| `bayes/` | Bayesian inference, MCMC, hierarchical models, prior elicitation | `tsa/`, `methods/` |
...
1. Read the source via `cc-markitdown`
2. Determine which sub-wikis it informs
...
```
Operations: `ingest`, `query`, `lint`, `project`.
